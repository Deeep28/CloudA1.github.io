...
def isvalid_location(location):
    locations = ['manhattan', 'brooklyn', 'queens', 'sunset park', 'egewater', 'bensonhurst', 'jackson heights',
...
def isvalid_email(email):
    ...
    EMAIL_REGEX = re.compile(
        r"""(?:[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#'$""" + r"""%&*+\/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d""" + r"""-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*""" + r"""[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][""" + r"""0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[""" + r"""a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|""" + r"""\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])""")
...
def make_restaurant_reservation(intent_request):
    ...
    mess = f'Alright, you are all set. Please let me repeat your preference: You would like to have {cuisine} restaurant suggestions for {numberpeople} people in {location} on {diningdate}, at {diningtime} And your Email ID is {email}. Expect my suggestions shortly! Have a good day.'
